...
function today(){
 ...
 const days=Array.from({length:7},(_,i)=>{const d=new Date();d.setDate(d.getDate()-1+i);return `<span class="day-pill ${i===1?'active':''}"><b>${d.toLocaleDateString('fr-CA',{weekday:'short'}).slice(0,1).toUpperCase()}</b><em>${d.getDate()}</em></span>`}).join('');
 return `<header class="today-head"><div><h1>Bonjour Boris</h1><p>Que la Parole devienne un réflexe.</p></div><span class="today-mark" aria-hidden="true">IÉ</span></header><div class="week-strip">${days}</div><section class="daily-card"><div class="daily-content"><div class="daily-top"><span class="card-kicker">SESSION DU JOUR · ${L.lesson.ref}</span><span class="daily-ring">${Math.min(100,Math.round((progress.active?.step||0)/6*100))}%</span></div><div class="daily-copy"><h2>${progress.active?'Reprenons là où tu en étais.':'Faire confiance à Dieu'}</h2><p>S’appuyer sur Sa sagesse, même lorsque le chemin semble incertain.</p></div><div class="daily-actions"><button class="listen-button" data-action="listen-session">▶ <span>Écouter</span></button>${button(progress.active?'Reprendre':due?'Réviser':'Commencer','start-session','light-button')}</div></div></section><section class="review-card"><span class="review-book">▤</span><div><strong>${due?'Révision du jour':'Révision'}</strong><small>${review?`${L.lesson.ref} · ${review.state}`:'4 passages à consolider'}</small></div><button data-action="start-session" aria-label="Ouvrir la révision">›</button></section><section><div class="section-title"><h2>Ma progression</h2><span>${accountSync?'Mon compte':'Sur cet appareil'}</span></div><div class="stats"><article><small>Sessions</small><strong>${metrics.sessions}</strong></article><article><small>Passages</small><strong>${metrics.encountered}</strong></article><article><small>Consolidés</small><strong>${metrics.retained}</strong></article><article><small>Objectif</small><strong>${esc(progress.preferences.minutes)}</strong></article></div></section>`;
}
...
function pathsPage(){ const [featured,...others]=paths; return `<header class="page-head paths-head"><h1>Parcours</h1><p>Grandir, un pas après l’autre.</p></header><section class="featured-path"><div class="featured-path-copy"><span>PARCOURS GUIDÉ</span><h2>${featured.title}</h2><p>Jour 8 sur 30</p><div class="path-progress"><div><i style="width:${featured.progress}%"></i></div></div><small>Bâtir sa vie sur les vérités de la Parole, jour après jour.</small></div><button data-action="path-open" aria-label="Continuer ${featured.title}">›</button></section><div class="path-list modern">${others.map((p,i)=>`<article class="path-${i+2}"><div class="path-thumb" aria-hidden="true"></div><div class="path-content"><div><h2>${p.title}</h2><span>${p.days}</span><p>${p.skill}</p></div></div><button data-action="path-open" aria-label="Ouvrir ${p.title}">›</button></article>`).join('')}</div>`; }
function profile(){
...
function action(a){
 ...
 if(a==='listen-session'){notify('La version audio arrive bientôt.');return;}
